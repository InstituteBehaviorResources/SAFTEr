...
#' @param dot Add a dot for each data-point along the line. Defaults to FALSE
...
#' @param print Print the plot after function run. Defaults to TRUE
#'
#'
  ...
  function(dataset,
    ...
    suppressWarnings({
      suppressMessages({
        ...
        cmp_data<-Compare_Plot %>%
          drop_na(compare_col)

        ### Plots for SAFTE model item and compare data
        p1<-ggplot() +
          #plot line for SAFTE iteam plot
          geom_line(data = Compare_Plot, aes(x=Fraction_Days, y = Compare_Plot$plot_var), size = 1.25, color = "dark blue") +

          #plot line for Compare data plot
          geom_point(data = cmp_data,aes(x=Fraction_Days, y =  a + compare_col*b), color = "red", size = 2) +

          #plot line for Compare data plot
          geom_line(data = cmp_data,aes(x=Fraction_Days, y =  a + compare_col*b), color = "red", size = .8) +
